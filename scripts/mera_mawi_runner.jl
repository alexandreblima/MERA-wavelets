...
function parse_cli()
    ...
    @add_arg_table! s begin
        ...
        "--analysis-wavelet"; arg_type=String; default="db4"; help="Wavelet utilizada para métricas Hurst/multifractal (aliases: db4, c3, s8, haar, ...)"
    end
    return parse_args(s)
end
...
function run_window_optimized(window_id, signal, cfg)
    try
        ...
        L = cfg["mera-L"]
        # Reproducibility metadata (propagated from main)
        seed = get(cfg, "seed", missing)
        git_commit = get(cfg, "git_commit", missing)
        git_dirty = get(cfg, "git_dirty", missing)
        manifest_sha = get(cfg, "manifest_sha256", missing)
...
function main()
    ...
    # Repo metadata (commit, manifest hash)
    ...
    # Propagate reproducibility metadata to window runner via cfg dict
    args["seed"] = args["seed"]
